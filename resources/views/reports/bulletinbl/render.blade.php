        ...
        <table class="services">
            ...
            @foreach ($days as $day)
                <tr class="@if($loop->index % 2) highlight-row @endif">
                    <td>
                       <b>{{ \Carbon\Carbon::parse($day)->isoFormat('DD. MMMM') }},<br/>
            ...
            <div class="infobox">
                @foreach ($specialServices as $group)
                    ...
                    @foreach($group['services'] as $service)
                            <li>
                                {{ $service->date->isoFormat('dddd, DD. MMMM') }}{!! ((!$group['options']['sameTime']) || (count($group['services']) == 1)) ? ', '.$service->timeText(true, '.', false, true) : '' !!}{{ ((!$group['options']['sameLocation']) && (trim($service->locationText()) != '')) ? ', '.$service->locationText() : ''  }}
                            </li>
                    @endforeach
                    </ul>
                @endforeach
