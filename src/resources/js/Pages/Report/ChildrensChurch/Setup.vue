...
<template>
    <admin-layout title="Programm für die Kinderkirche erstellen">
        ...
        <form method="post" :action="route('reports.render', {report: 'childrensChurch'})" @submit.prevent="renderReport">
            <form-csrf-token />
...
export default {
    ...
    methods: {
        renderReport() {
            submitReportForm(route('reports.render', {report: 'childrensChurch'}), {
                city: this.myCity,
                start: this.from,
                end: this.to,
            });
